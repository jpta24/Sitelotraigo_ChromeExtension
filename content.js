...
function getMedNumByClass(classType) {
	var uniMedida = document.getElementsByClassName(classType)[postm0]
...
function getTipoEnvByClass(classType) {
	var uniPeso = document.getElementsByClassName(classType)[postm0]
		.nextElementSibling.innerHTML;
	var i;
...
function getPesoByClass(classType) {
	var uniPeso = document.getElementsByClassName(classType)[postm1]
		.nextElementSibling.innerHTML;
...
function calculosIniciales() {
	...
	item.medidas.volumen =
		item.medidas.largo * 1 * (item.medidas.ancho * 1) * (item.medidas.prof * 1);
	...
	if (item.precioxCant * 0.6 < 200) {
		...
		item.costoEnvio.impuesto =
			item.precioxCant * 0.6 * item.costoEnvio.porcentImpuesto;
	}
